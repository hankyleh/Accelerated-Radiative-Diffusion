...
In the Python code, the global matrix assembly from local matrices is performed by the function {\pythoninline/method.assemble_global_matrix()/}, and the matrix definition can be found in {\pythoninline/tools/}. For example, the interior assembly:
...
\subsection{Test A1, steady-state solution}
...
\begin{gather}
    I^{in, +}= \frac{1}{2}\beta_k(T_0)\\
    I^{in, -}= \frac{1}{2}\beta_k(T_0)\\
    F^{in, +}= \frac{1}{4}\beta_k(T_0)\\
    F^{in, -}= -\frac{1}{4}\beta_k(T_0)\\
    I(x)|_{t=0} = \beta_k(T_0)\\
    F(x)|_{t=0} = 0.
\end{gather}
...
\dirtree{%
    ...
    .3 solve\_diffusion()\descript{handles time advancement. At each time step, call appropriate ...loop(), record results}.
